...
# Запуск контейнеров в cardinal

Это практическое руководство по ежедневной работе: установка cardinal, загрузка образа, запуск приложения, подключение постоянных файлов, `.env`, просмотр логов, обновление кода и решение типичных ошибок.

> cardinal запускает контейнеры на Linux. Команды ниже рассчитаны на Bash и выполняются от `root` либо с необходимыми привилегиями.
...
## 2. Установка и обновление cardinal
...
потому что cardinal требует namespace, mount, cgroup и сетевые возможности хоста.
...
Для ARM64 используйте matching `cardinal-*-linux-arm64.AppImage` asset. AppImage не
требует пакетного менеджера, но cardinal всё равно требует от хоста Linux
...
использовать бинарник `cardinal-linux-armv6` или архив `.tar.gz`.

#### Установка двойным кликом на Linux Desktop

AppImage cardinal — это CLI-runtime, а не графическое приложение. При двойном
...
бинарник cardinal в `/usr/local/bin/cardinal`, при необходимости запросит права
администратора и установит/запустит `cardinal-bootstrap.service`, если доступен
systemd. Существующие контейнеры и образы в каталоге данных cardinal сохраняются,
...
команду cardinal. Этот блок сам найдёт скачанный файл в текущем каталоге:
...
Если cardinal запущен из AppImage, изменить read-only mount самого AppImage
невозможно. Теперь updater устанавливает проверенный новый статический
бинарник в `/usr/local/bin/cardinal`, а исходный AppImage оставляет без изменений.
...
Если `cardinal update` не может скачать бинарник (в старых версиях — ошибка `Failed to download binary: all methods failed`), установите релиз вручную. Подставьте нужную версию и архитектуру:
...
Для workflow pull → verify → run проверяйте целостность образа после скачивания и перед запуском. `cardinal verify` сверяет config и диджесты всех слоёв с сохранённым манифестом локально, без обращения к registry:
...
`cardinal verify` завершится с ненулевым кодом, если образ отсутствует локально, диджест config не совпадает с сохранённой мета-информацией или манифестом, либо повреждён какой-либо файл слоя. Это быстрая офлайн-проверка для образов, восстановленных через `cardinal import` или перенесённых между хостами.
...
Доступные политики перезапуска: `no`, `always`, `on-failure`, `unless-stopped`. Добавьте `--restart-delay 1m` (или короткое значение `10s`), чтобы задать задержку перед автоматическим запуском после неожиданного завершения процесса. Намеренный `cardinal stop` эту задержку не отменяет и контейнер не запускает.

Частые быстрые падения защищены: когда исчерпан бюджет crash-loop (по умолчанию 5 перезапусков; настраивается через `--restart-max-attempts` и `--restart-window`), автоматический перезапуск блокируется — `cardinal inspect ИМЯ` покажет `"restart_blocked": true` — и контейнер остаётся остановленным до явного `cardinal start`.
...
В целях безопасности cardinal запрещает bind source, которые указывают на чувствительные каталоги хоста: `/`, `/root`, `/etc`, `/proc`, `/sys` и другие системные пути. Поэтому каталог вроде `/root/myapp` может потребоваться перенести в `/data/myapp` или другой отдельный каталог.

> Исходный каталог на хосте должен существовать до `cardinal run`. Команда `--vol "$PWD:/app"` корректна только тогда, когда текущий каталог разрешён для bind mount.
...
```bash
cardinal run -d -e APP_ENV=production -e PORT=8080 ОБРАЗ[:ТЕГ] КОМАНДА
```
...
chmod 600 .env
cardinal run -d --env-file .env ОБРАЗ[:ТЕГ] КОМАНДА
```
...
DNS в bridge-режиме работает из коробки (дефолт `8.8.8.8`/`8.8.4.4`, переопределить — флагом `--dns`). Используйте `--network host`, только если сервер должен слушать хостовые интерфейсы напрямую. Сервер должен слушать `0.0.0.0:25565`, а не только `127.0.0.1`.
...
```bash
cardinal ps                 # работающие контейнеры
cardinal ps -a              # работающие и остановленные
cardinal stop ИМЯ           # остановить без удаления данных
cardinal start ИМЯ          # запустить существующий остановленный
cardinal restart ИМЯ       # остановить и запустить
cardinal rm ИМЯ             # удалить остановленный
cardinal rm -f ИМЯ          # принудительно удалить
```

`cardinal stop` сохраняет overlay контейнера и данные bind mount. `cardinal rm -f` удаляет overlay и внутренние state/log-файлы cardinal. Файлы bind mount и данные именованных томов при удалении контейнера не удаляются.

## 10. Логи и attach

Для root-установки cardinal лог stdout/stderr контейнера хранится здесь:

```text
/root/.cardinal/logs/<container-id>.log
```

Корень данных можно изменить через `CARDINAL_DATA_DIR`:
...
```bash
cardinal logs ИМЯ
cardinal logs --tail 100 ИМЯ
cardinal logs -f ИМЯ
cardinal attach ИМЯ
```

`cardinal logs -f` показывает новые строки. `cardinal attach` подключается к главному процессу detached-контейнера. Для выхода из attach нажмите `Ctrl+C` — контейнер не остановится.

При новом запуске cardinal создаёт свежий stdout/stderr лог, поэтому вывод прошлых запусков не накапливается после `stop`/`start` или `restart`. Логи самого приложения — отдельная вещь: например, Minecraft `/data/logs/latest.log` хранится в bind mount или именованном томе и сохраняется между запусками.

## 11. Автоматические бэкапы

Для каждого контейнера можно включить собственное расписание. Архив содержит writable overlay и именованные тома, но не host bind mount. Каталоги вроде `/data/minecraft` нужно архивировать отдельно. Чтобы данные были согласованными, cardinal ненадолго остановит работающий контейнер, создаст архив и запустит его снова. При включении расписания архив сразу не создаётся: первый архив появится после заданного интервала.
...
По умолчанию архивы сохраняются в `$CARDINAL_DATA_DIR/backups/<container>/`. При необходимости укажите отдельный каталог:
...
Разовый бэкап можно создать командой `cardinal backup create ИМЯ`; перед этим остановите контейнер. Восстанавливать архив можно только в остановленный контейнер. Ручные и автоматические архивы включают данные overlay и именованных томов cardinal, но не host bind mount:

```bash
cardinal backup restore minecraft /data/backups/minecraft/minecraft-20260811-120000.tar.gz
```

Настройки авто-бэкапа хранятся в state контейнера и переживают `stop`, `start` и обновление cardinal. Retention удаляет самые старые архивы расписания после успешного бэкапа; файлы с другими именами он не трогает.

Архив можно проверить по контрольной сумме: `cardinal backup verify ФАЙЛ.tar.gz`. Если checksum-файла рядом нет, cardinal сообщит, что архив валиден, но не проверен.

## 12. Проверка и команды внутри контейнера

```bash
cardinal exec ИМЯ команда аргументы...
cardinal exec -i -t ИМЯ /bin/sh
cardinal console ИМЯ
cardinal top ИМЯ
cardinal port ИМЯ
cardinal stats ИМЯ --no-stream
cardinal fs ls ИМЯ /путь
cardinal fs cat ИМЯ /путь/файла
cardinal cp ./локальный-файл ИМЯ:/путь/
cardinal cp ИМЯ:/путь/файла ./локальный-файл
```
...
```bash
cardinal run -d --memory 512m --cpus 1 --disk 5G ОБРАЗ[:ТЕГ] КОМАНДА
cardinal run -d --user 1000:1000 --cap-drop ALL --no-new-privs ОБРАЗ[:ТЕГ] КОМАНДА
cardinal run -d --readonly ОБРАЗ[:ТЕГ] КОМАНДА
```
...
Все флаги cardinal размещайте до имени образа. Всё после образа и команды передаётся процессу внутри контейнера. Например, `-p 23323:23332` должен находиться до `python:3.12`, а не после `sh -c`.

### Политики перезапуска

| Политика | Процесс неожиданно завершился | `cardinal stop` | Перезагрузка хоста |
|---|---|---|---|
...
| `unless-stopped` | Перезапуск | Останется остановленным до явного `cardinal start` | Запускается автоматически, если не был остановлен вручную |

`cardinal run --restart always` и `cardinal run --restart unless-stopped`, запущенные от root, автоматически устанавливают systemd-службу bootstrap. `--restart-delay` влияет на восстановление после завершения процесса и не задерживает первоначальный запуск после reboot. Установить bootstrap вручную можно так:
...
Minecraft должен слушать `0.0.0.0:25565`. Его собственные логи и миры сохраняются в `/data/minecraft`, а stdout/stderr cardinal очищается при каждом новом запуске контейнера.
...
Сбой процесса обрабатывается настроенной политикой перезапуска. Ручной `cardinal stop` считается намеренной остановкой и не даёт `unless-stopped` запуститься снова до `cardinal start`. Чтобы полностью отключить автоматическое восстановление:
...
```bash
cardinal run -d -n app --vol /data/app:/app --workdir /app ОБРАЗ[:ТЕГ] КОМАНДА
```
...
### `Failed to download binary: all methods failed` при `cardinal update`
...
Старые версии считали zombie-процессы (defunct) живыми, поэтому supervisor не замечал выход — контейнер застревал в `running`, рестарты почти не срабатывали, а ресурсы не очищались. Обновите cardinal и проверьте:

```bash
cardinal version
cardinal ps -a
cardinal inspect ИМЯ | grep -E '"status"|"pid"'
```

### Контейнер `--network none` «висит» перед запуском команды

При `--network none` интерфейс `eth0` не должен появляться, поэтому cardinal не ждёт его. При `--network host` уже используется интерфейс хоста, а в bridge-режиме cardinal недолго ждёт появления veth. Если запуск всё равно зависает, обновите cardinal и проверьте логи контейнера.

## 17. Где хранятся данные

Для root стандартный каталог данных cardinal — `/root/.cardinal`. Точный путь также показывает `cardinal info`:
...
Чтобы использовать другое место для внутреннего состояния, задайте `CARDINAL_DATA_DIR` до запуска cardinal. Bind mount проекта, например `/data/alfheimguide`, хранится отдельно от этого внутреннего каталога.
