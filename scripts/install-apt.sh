...
# Usage: curl -sSL https://raw.githubusercontent.com/kuranix/cardinal/main/scripts/install-apt.sh | sudo bash
set -eu
...
LATEST=$(curl -fsSL https://api.github.com/repos/kuranix/cardinal/releases/latest 2>/dev/null | \
...
DEB_BASE="https://github.com/kuranix/cardinal/releases/download/${LATEST}/cardinal_${LATEST}_amd64.deb"
TMPDIR=$(mktemp -d)
trap 'rm -rf "$TMPDIR"' EXIT

cd "$TMPDIR"
if ! curl -fsSL -o cardinal.deb "$DEB_BASE"; then
    DEB_BASE="https://github.com/kuranix/cardinal/releases/download/v${LATEST}/cardinal_${LATEST}_amd64.deb"
    curl -fsSL -o cardinal.deb "$DEB_BASE" || err "Failed to download: $DEB_BASE"
fi

# SHA256 verification against published SHA256SUMS.txt.
SUMS_URL="https://github.com/kuranix/cardinal/releases/download/${LATEST}/SHA256SUMS.txt"
